...
export const FinancialInsights: React.FC<FinancialInsightsProps> = ({ stats, chartData, language }) => {
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                <StatCard
                    ...
                    label={language === 'es' ? 'Ingreso Neto' : 'Net Revenue'}
                ...
                <StatCard
                    ...
                    label={language === 'es' ? 'Cola de Pedidos' : 'Order Queue'}
                ...
                <StatCard
                    ...
                    label={language === 'es' ? 'Facturación' : 'Invoices'}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Sales Chart */}
                <div className="lg:col-span-2 glass p-8 rounded-[3rem] border-white/5 relative overflow-hidden group">
                    ...
                    <div className="flex items-center justify-between mb-8 relative z-10">
                        <div>
                            <h3 className="text-xl font-black uppercase italic tracking-tighter">
                                {language === 'es' ? 'Rendimiento Táctico' : 'Tactical Performance'}
                            </h3>
                            <p className="text-[10px] font-black uppercase tracking-widest text-white/30">
                                {language === 'es' ? 'Ingresos netos por día' : 'Daily net revenue'}
                            </p>
                        </div>
                    </div>
                ...
                {/* Earnings Breakdown */}
                <div className="glass p-8 rounded-[3rem] border-white/5 flex flex-col justify-between relative overflow-hidden">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-end p-4 bg-white/5 rounded-2xl border border-white/5">
                                <span className="text-[10px] font-black uppercase text-white/40 tracking-widest">
                                    {language === 'es' ? 'Venta Bruta' : 'Gross Sales'}
                                </span>
                            ...
                            <div className="flex justify-between items-end p-4 bg-red-500/5 rounded-2xl border border-red-500/10">
                                <span className="text-[10px] font-black uppercase text-red-500/60 tracking-widest">
                                    {language === 'es' ? 'Comisión (5%)' : 'Commission (5%)'}
                                </span>
                            ...
                            <div className="pt-4 mt-4 border-t border-white/10">
                                <div className="flex justify-between items-end px-2">
                                    <span className="text-[10px] font-black uppercase text-primary tracking-widest">
                                        {language === 'es' ? 'Tu Balance' : 'Your Balance'}
                                    </span>
                    ...
                    >
                        {language === 'es' ? 'Ver Detalles de Pago' : 'View Payout Details'}
...
const StatCard = ({ icon, label, value, color = "text-white", trend, glow = "" }: StatCardProps) => (
    <motion.div
        ...
        className={`glass p-8 rounded-[2.5rem] border-white/5 flex items-center gap-6 transition-all duration-500 relative overflow-hidden group ${glow}`}
    >
        <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-full blur-3xl -mr-16 -mt-16 group-hover:bg-primary/10 transition-colors" />
        <div className="w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center text-primary border border-white/10 shadow-inner">
            {icon}
        </div>
