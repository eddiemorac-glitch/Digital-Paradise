...
export const QuickStockToggle: React.FC<QuickStockToggleProps> = ({ merchantId }) => {
        ...
        <div className="glass h-full rounded-[2.5rem] border-white/5 flex flex-col overflow-hidden">
            <div className="p-6 border-b border-white/5">
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-sm font-black uppercase italic tracking-widest text-white whitespace-nowrap">
                        {t('quick_stock')}
                    </h3>
                </div>
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full pl-9 pr-4 py-3 bg-white/5 border border-white/5 rounded-xl text-xs text-white placeholder:text-white/20 focus:outline-none focus:border-primary/30 transition-all font-black uppercase tracking-widest"
                    />
                </div>
            </div>

            <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar touch-pan-y">
                {isLoading ? (
                    <div className="flex flex-col items-center justify-center py-10 gap-2 opacity-20">
                        <Loader2 size={24} className="animate-spin" />
                        <span className="text-[8px] font-black uppercase tracking-widest">{t('syncing')}</span>
                    </div>
                        ...
                        {filteredProducts.map((product) => (
                            ...
                            >
                                ...
                                <button
                                    ...
                                    className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${product.isAvailable ? 'bg-primary/20 text-primary' : 'bg-red-500/20 text-red-500'}`}
                                >
